import { tableName } from "@softwaretechnik/dbml-renderer/lib/common";
...
class TableNameRenderer {
  ...
  toDot() {
    const tableColor = !this.table.settings?.headercolor ? "#1d71b8" : this.table.settings.headercolor;
...
class CompositeKeyRowRenderer {
  ...
  toDot() {
    return `<TR><TD PORT="${this.port}" BGCOLOR="#e7e2dd"><FONT COLOR="#1d71b8"><I>    ${this.columns.join(
      ", ",
    )}    </I></FONT></TD></TR>`;
  }
}
...
class TableRenderer {
  constructor(table) {
    ...
    this.renderers = [new TableNameRenderer(table.actual, this.displayName()), ...this.columns];
  }
    ...
    columns.forEach(
      (columnName) =>
        (columnIndex[columnName] =
          this.renderers.findIndex((c) => c.name === columnName) + 1 || Number.MAX_SAFE_INTEGER),
    );
    const name = columns.sort((a, b) => columnIndex[a] - columnIndex[b]).join(",");
  ...
  toDot() {
    ...
    const tooltip = !note ? "" : `tooltip="${this.displayName()}\\n${escapeString(note)}";`;
...
export class DbmlRenderer {
  constructor(dbml) {
    ...
    const allTables = this.groups.flatMap((group) => group.tables).concat(this.ungroupedTables);
